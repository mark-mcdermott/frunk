...
<div class="min-h-screen bg-surface-50 dark:bg-surface-900 flex flex-col">
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
		<Breadcrumbs items={[{ label: 'Vehicles', href: `${basePath}/vehicles` }, { label: 'Add Vehicle' }]} />
	</div>
		...
		<div class="bg-white dark:bg-surface-800 rounded-2xl p-6 shadow-xl shadow-surface-900/5 w-full max-w-sm border border-[#eee]">
			...
			<form method="post" use:enhance class="space-y-4">
				...
				<div class="flex gap-3">
					...
					<a href="{basePath}/vehicles" class="flex-1 btn preset-outlined-surface-500 py-2.5 rounded-lg font-semibold block text-center border border-surface-300 dark:border-surface-600 hover:bg-surface-100 dark:hover:bg-surface-700">
						Cancel
					</a>
				</div>
			</form>
