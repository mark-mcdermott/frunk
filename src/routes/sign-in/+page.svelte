...
<div class="min-h-screen bg-surface-50 dark:bg-surface-900 flex flex-col">
	...
	<main class="flex-1 flex items-start justify-center pt-12 px-4 sm:px-6 lg:px-8">
		<div class="bg-white dark:bg-surface-800 rounded-2xl p-6 shadow-xl shadow-surface-900/5 w-full max-w-sm border border-[#eee]">
			...
			<p class="text-sm text-surface-500 dark:text-gray-300 mb-6">Complete the form to sign in.</p>
			...
			<form method="post" action="?/login" use:enhance class="space-y-4">
				...
				<p class="text-xs text-surface-500 dark:text-gray-300 text-right">Don't have an account? <a href="/sign-up" class="text-primary-500 hover:text-[#93c5fd] transition-colors">Sign up.</a></p>
			</form>
			{#if form?.message}
