				...
				<div class="grid gap-4">
					{#each vehicles as vehicle}
						<div
							...
							onclick={(e) => { if (e.target.closest('a, button, form')) return; goto(`/vehicles/${vehicle.id}`); }}
							role="button"
							tabindex="0"
							onkeydown={(e) => { if (e.key === 'Enter' && !e.target.closest('a, button, form')) goto(`/vehicles/${vehicle.id}`); }}
						>
